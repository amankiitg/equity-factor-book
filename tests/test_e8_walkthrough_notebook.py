"""Sprint E8 Task 10: the walkthrough, its render and the docs links."""
...
NOTEBOOK = ROOT / "notebooks" / "E8_walkthrough.ipynb"
HTML = ROOT / "notebooks" / "E8_walkthrough.html"
RESULTS = ROOT / "sprints" / "E8" / "RESULTS.json"

CRITERIA = [f"F8.{index}" for index in range(1, 7)]


def _notebook() -> dict:
    if not NOTEBOOK.exists():
        pytest.skip("E8 walkthrough not written yet")
...
def test_walkthrough_has_been_executed() -> None:
    ...
    assert len(executed) >= 9, "the walkthrough has not been executed"
...
def test_the_hash_cell_asserts_the_sprint_hash_against_the_artifacts() -> None:
    ...
    assert "e8_data_hash" in joined, "the hash is not recomputed in the notebook"
