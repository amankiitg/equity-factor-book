"""Sprint E6 Task 4: the stored results file is complete and reproducible.
...
def _results() -> dict:
    if not RESULTS.exists():
        pytest.skip("E6 results not written yet")
...
def test_the_stored_criteria_equal_the_recomputed_ones() -> None:
    payload = _results()
    fresh = evaluate.evaluate_e6_criteria(**evaluate.compute_e6_from_artifacts())
    for name in CRITERIA:
        assert (
...
def test_the_criterion_text_is_never_reworded() -> None:
    ...
    for name in CRITERIA:
        assert (
            payload["criteria"][name]["criterion"] == evaluate.E6_CRITERIA_TEXT[name]
        ), name
